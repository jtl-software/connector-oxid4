...
class PrimaryKeyMapper implements IPrimaryKeyMapper
{
    ...
    {
        $dbResult = $this->db->getOne('SELECT hostId FROM jtl_connector_link WHERE endpointId = "'.$endpointId.'" AND type = '.$type);
    ...
    {
        $dbResult = $this->db->getOne('SELECT endpointId FROM jtl_connector_link WHERE hostId = '.$hostId.' AND type = '.$type);

        $endpointId = $dbResult ? $dbResult : null;

        Logger::write(sprintf('Trying to get endpointId with hostId (%s) and type (%s) and realtion type (%s)... endpointId: (%s)', $hostId, $type, $relationType, $endpointId), Logger::DEBUG, 'linker');
